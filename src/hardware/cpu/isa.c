...
static uint64_t compute_operand(od_t *od)
{
    ...
    {
        // access memory: return virtual address
        uint64_t vaddr = 0;
...
static void parse_operand(const char *str, od_t *od)
{
    // str: assembly instruction operand string, e.g. $0x18, %rsp
...
// handler table mapping the handlers into different instruction types
...
static void pop_handler(od_t *src_od, od_t *dst_od)
{
    ...
    if (src_od->type == REG)
    {
        // src: register
        // dst: empty
        uint64_t old_val = read64bits_dram(va2pa(cpu_reg.rsp));
        cpu_reg.rsp = cpu_reg.rsp + 8;
...
static void call_handler(od_t *src_od, od_t *dst_od)
{
    ...
    // src: (immediate number) virtual address of target function
    // dst: empty
...
static void add_handler(od_t *src_od, od_t *dst_od)
{
    ...
    if (src_od->type == REG && dst_od->type == REG)
    {
        // src: register (value: int64_t bitmap)
        // dst: register (value: int64_t bitmap)
        uint64_t val = *(uint64_t *)dst + *(uint64_t *)src;
    
        // set condition flags
        uint64_t sign_src = (*(uint64_t *)src >> 63) & 0x1;
        uint64_t sign_dst = (*(uint64_t *)dst >> 63) & 0x1;
        uint64_t sign_val = (val >> 63) & 0x1;

        cpu_flags.CF = (val < *(uint64_t *)src); // unsigned overflow
        cpu_flags.ZF = (val == 0);
        cpu_flags.SF = sign_val;
        // pos + neg will not overflow
        // signed overflow: pos + pos = neg, neg + neg = pos
        cpu_flags.OF = (!(sign_src ^ sign_dst) && (sign_val ^ sign_src));
...
static void sub_handler(od_t *src_od, od_t *dst_od)
{
    ...
    if (src_od->type == IMM && dst_od->type == REG)
    {
        ...
        // set condition flags
        uint64_t sign_val = (val >> 63) & 0x1;
        uint64_t sign_src = (src >> 63) & 0x1;
        uint64_t sign_dst = (*(uint64_t *)dst >> 63) & 0x1;

        cpu_flags.CF = (val > *(uint64_t *)dst);
        cpu_flags.ZF = (val == 0);
        cpu_flags.SF = sign_val;
        // signed overflow: pos - neg = neg, neg - pos = pos
        cpu_flags.OF = (sign_dst == 0 && sign_src == 1 && sign_val == 1) || 
                       (sign_dst == 1 && sign_src == 0 && sign_val == 0);
...
void instruction_cycle()
{
    ...
    // update CPU and memory according to the instruction
